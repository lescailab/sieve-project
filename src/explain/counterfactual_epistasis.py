"""
Counterfactual epistasis detection for variant interactions.

Uses attention pre-filtering combined with counterfactual perturbation to
identify and validate epistatic interactions.  The method is NOT based on
Shapley interaction values; individual variants can optionally be scored
with marginal SHAP values, but pairwise interaction SHAP values are not
computed.

Workflow:
1. Estimate candidate pairs from attention weights × attribution scores.
2. Validate each candidate with a four-condition counterfactual perturbation
   experiment (both present, only v1, only v2, neither present).
3. Classify interactions as synergistic, antagonistic, or independent based
   on the deviation from additivity.
...
class CounterfactualEpistasisDetector:
    """
    Detect epistatic interactions using attention pre-filtering and
    counterfactual perturbation.

    This detector identifies candidate variant pairs with high mutual
    attention and high individual attribution scores, then validates
    them using a four-condition counterfactual perturbation experiment.

    **Important**: This class does NOT compute pairwise Shapley interaction
    values.  Individual variants can be scored with marginal SHAP values
    via :meth:`compute_variant_shap_values`, but the pairwise interaction
    scoring is done through the counterfactual procedure in
    :meth:`validate_interaction_with_perturbation`.
    ...
    background_samples : int
        Number of background samples for marginal SHAP scoring (default: 50)

    Examples
    --------
    >>> detector = CounterfactualEpistasisDetector(model, device='cuda')
    >>> interactions = detector.estimate_interactions_from_attention(
    ...     attention_weights, attributions, positions, gene_ids, mask
    ... )
    >>> for pair in interactions[:10]:
    ...     result = detector.validate_interaction_with_perturbation(
    ...         features, positions, gene_ids, mask,
    ...         pair['variant1_idx'], pair['variant2_idx']
    ...     )
    """
    ...
    def create_background_data(
        ...
        """
        Create background dataset for marginal SHAP scoring.
    ...
    def compute_variant_shap_values(
        ...
        This scores each variant independently.  It does NOT produce
        pairwise interaction SHAP values.

        Due to computational cost, SHAP is computed for the top K variants
        ...
        mask : Tensor
            Validity mask
        background : Tuple
            Background data for SHAP (from :meth:`create_background_data`)
        max_variants : int
            Maximum number of variants to analyse
    ...
    def estimate_interactions_from_attention(
        ...
        """
        Fast pre-filtering of candidate interactions using attention × attribution.

        Combines attention patterns (which variants attend to each other)
        with attribution scores (which variants are important) to prioritise
        likely epistatic interactions for counterfactual validation.

        Interaction score = attention_weight × (importance_i + importance_j)

        This score is a heuristic for candidate ranking, NOT an estimate of
        interaction strength.  Use :meth:`validate_interaction_with_perturbation`
        ...
        mask : Tensor
            Validity mask
        top_k : int
            Number of top candidate pairs to return

        Returns
        -------
        interactions : List[Dict]
            Prioritised candidate pairs for counterfactual validation
        """
    ...
    def validate_interaction_with_perturbation(
        ...
        validation : Dict
            ...
            - interaction_type ('synergistic', 'antagonistic', or 'independent')
            - is_significant: whether |synergy| > 0.05
        """
        self.model.eval()
        ...
        # Positive synergy = synergistic (combined effect > sum of individual effects)
        # Negative synergy = antagonistic (combined effect < sum of individual effects)
