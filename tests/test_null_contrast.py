...
Unit tests for the null-contrast ranking pipeline:
  - compare_attributions.py  (new empirical p-value / FDR logic)
...
def _make_corrected_df(
    z_values: np.ndarray,
    chromosomes: list[str] | None = None,
    gene_names: list[str] | None = None,
    seed: int = 0,
) -> pd.DataFrame:
    """Build a minimal corrected variant rankings DataFrame."""
    n = len(z_values)
    ...
    return pd.DataFrame({
        ...
        'mean_attribution': z_values * 0.01 + 0.05,
        'z_attribution': z_values,
        'corrected_rank': np.argsort(-z_values) + 1,
        'is_sex_chrom': False,
        'chromosome_mean': 0.05,
        'chromosome_std': 0.01,
...
class TestComputeEmpiricalPvalues:
    ...
    def test_minimum_p_equals_one_over_n_plus_one(self) -> None:
        """
        If the highest real z_attribution exceeds all null values the empirical
...
class TestCompareAttributionsMain:
    ...
    def _write_corrected(self, df: pd.DataFrame, path: Path) -> None:
    ...
    def test_outputs_three_files(self, tmp_path: Path) -> None:
        ...
        real_z = rng.normal(0.5, 1, size=300)
        null_z = rng.normal(0, 1, size=400)

        real_df = _make_corrected_df(real_z)
        null_df = _make_corrected_df(null_z)

        real_path = tmp_path / 'real_corrected.csv'
        null_path = tmp_path / 'null_corrected.csv'
        self._write_corrected(real_df, real_path)
        self._write_corrected(null_df, null_path)

        import sys as _sys
        old_argv = _sys.argv[:]
        _sys.argv = [
            'compare_attributions.py',
            '--corrected-real', str(real_path),
            '--corrected-null', str(null_path),
        ...
        out = tmp_path / 'output'
        assert (out / 'corrected_variant_rankings_with_significance.csv').exists()
        assert (out / 'corrected_gene_rankings_with_significance.csv').exists()
    ...
    def test_augmented_variant_file_has_significance_columns(
        ...
        real_df = _make_corrected_df(rng.normal(0.5, 1, size=200))
        null_df = _make_corrected_df(rng.normal(0, 1, size=300))
        ...
        out_df = pd.read_csv(tmp_path / 'out' / 'corrected_variant_rankings_with_significance.csv')
    ...
    def test_gene_file_has_significance_columns(self, tmp_path: Path) -> None:
        ...
        gene_df = pd.read_csv(
            tmp_path / 'out2' / 'corrected_gene_rankings_with_significance.csv'
        )
        assert 'empirical_p_gene' in gene_df.columns
        assert 'fdr_gene' in gene_df.columns

    def test_fails_if_z_attribution_missing_in_real(self, tmp_path: Path) -> None:
        """Script must raise ValueError if real file lacks z_attribution."""
        rng = np.random.default_rng(5)
        bad_real = pd.DataFrame({
            'chromosome': ['1'] * 10,
            'mean_attribution': rng.normal(size=10),
            # deliberately no z_attribution
        })
        null_df = _make_corrected_df(rng.normal(size=100))
        ...
        _sys.argv = [
            'compare_attributions.py',
            '--corrected-real', str(bad_path),
            '--corrected-null', str(null_path),
            '--output-dir', str(tmp_path / 'out3'),
        ]
        try:
            with pytest.raises(ValueError, match='z_attribution'):
                compare_mod.main()
        finally:
            _sys.argv = old_argv

    def test_fails_if_z_attribution_missing_in_null(self, tmp_path: Path) -> None:
        """Script must raise ValueError if null file lacks z_attribution."""
        rng = np.random.default_rng(6)
        real_df = _make_corrected_df(rng.normal(size=100))
        bad_null = pd.DataFrame({
            'chromosome': ['1'] * 50,
            'mean_attribution': rng.normal(size=50),
        })
